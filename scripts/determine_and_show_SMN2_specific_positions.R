...
SMN_specific_positions_bed <- SNV_tally_SMN_filtered %>%
  ungroup() %>%
  filter(copy_specific == smn_type) %>% #filter for only smn_type specific positions
...
#write bed files with SMN specific positions
write.table(SMN_specific_positions_bed, paste(prefix, "_", smn_type, "_specific_positions_0.9_SMN1_0.2.bed", sep = ""), col.names = FALSE, row.names = FALSE, sep="\t", quote=FALSE)

#Look at SNVs at smn_type specific positions, but for specific_positions_bed
colnames(SMN_specific_positions_bed) <- c("chr", "start", "POS", "annotation")
SNV_at_SMN_specific_positions <- SNV_table %>%
  filter(POS %in% SMN_specific_positions_bed$POS) %>% #filter for smn_type specific positions
...
#pivot the table to show all SNVs next to each other at SMN-specific positions
...
write_tsv(SNV_at_SMN_specific_positions_pivoted, paste(prefix, "_", smn_type, "_at_SMN2_specific_positions.tsv", sep = ""), quote="none", na="")
